...
import { Plus, X, ArrowUpRight, Filter, ChevronLeft, ChevronRight, FileDown } from 'lucide-react';
...
export default function StockExits() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [extRes, prodRes, catRes] = await Promise.all([
        api.get(`/stock-exits?page=${page}&category_id=${categoryFilter}`),
        api.get('/products?per_page=100'),
        api.get('/categories')
      ]);
      setExits(extRes.data.data);
      setTotalPages(extRes.data.last_page);
  ...
  const downloadCSV = async () => {
    try {
      const response = await api.get(`/stock-exits/export?category_id=${categoryFilter}`, {
        responseType: 'blob',
      });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'sorties_stock.csv');
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setShowModal(true)} className="bg-rose-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-colors hover:bg-rose-700 shadow-md"><Plus size={20} /> Nouvelle Sortie</button>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-4">
        <div className="flex items-center gap-2">
          ...
          <select 
            className="bg-slate-50 border border-slate-200 rounded-lg px-4 py-2 text-slate-600 outline-none focus:ring-2 focus:ring-rose-500"
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr><td colSpan="5" className="px-6 py-10 text-center text-slate-500">Aucune sortie trouvée</td></tr>
              ) : exits.map(e => (
